...
def extract_zip_to_temp(zip_path: Path) -> ExportPaths:
    ...
    return ExportPaths(work_dir=tmp_dir, md_files=md_files, csv_files=csv_files)
...
def import_benefits_csv(db: Session, csv_path: Path) -> None:
    ...
    if not col_benefit or not col_product:
        print(f"[WARN] benefits csv columns not detected: {rows[0].keys()}")
        return
    ...
    for r in rows:
        ...
        db.flush()
        upsert_product_benefit(db, p.id, b.id, note="", evidence_level=None)
...
def import_studies_csv(db: Session, csv_path: Path) -> None:
    ...
    if not col_product or not col_title:
        print(f"[WARN] studies csv columns not detected: {rows[0].keys()}")
        return
    ...
    for r in rows:
        ...
        db.flush()
        upsert_product_study(db, p.id, s.id, note="")


# ----------------------------------------
# Markdown import (products + stacks)
# ----------------------------------------
    ...
    for p in md_files:
        ...
        if "stack" in fname:
            ...
            for prod_name, val, unit in dosages:
                ...
                db.flush()
                upsert_stack_product(db, st.id, pr.id, val, unit, note="", sort_order=sort)
                sort += 10

            continue

        # --- product pages ---
        # Heuristique : fichiers md dans dossiers "produits/études/bienfaits/ingredients"
        if any(x in parent for x in ["produit", "product", "etude", "study", "ingredient", "bienfait"]):
            ...
            # Notion export often has a sibling image.png per product
            image_media_id = None
            img = p.parent / "image.png"
            if img.exists():
                content = img.read_bytes()
                m = upsert_media(db, content, img.name)
                image_media_id = m.id
            ...
            upsert_product(
                ...
                description_md=text,
                category="",
                image_path="",  # not used when storing in DB
...
def run_import(db: Session, zip_path: Path) -> dict:
    ...
    studies_csv = (
        pick_csv(exp.csv_files, "études")
        or pick_csv(exp.csv_files, "etudes")
        or pick_csv(exp.csv_files, "studies")
    )
...
def main() -> None:
    ...
    parser.add_argument("--zip", required=True, help="Path to Notion export zip")
